...
-- 向ui管理类注册面板类
function mgr:register( window_name, package_name, path, kind )
	local window_info = {
...
function mgr:open(window_name, callback)
	...
	if window_obj ~= nil then
		...
		-- 不同方式加载资源(优先AssetBundle，找不到再Resources)
		window_obj = AssetsSvc:LoadUIPrefab(info.package_name, info.window_name, self.EAssetsType.AssetBundle)
		if window_obj== nil then
			window_obj = AssetsSvc:LoadUIPrefab('', info.path, self.EAssetsType.Resources)
		end
		...
		-- 实例化物体，并加入缓存容器
		local window_rect = window_obj:GetComponent('RectTransform') 
		window_rect:SetParent(self:get_ui_root_transform())
		window_rect.anchoredPosition = UnityEngine.Vector2.zero -- 设置 left 和 top
		window_rect.sizeDelta = UnityEngine.Vector2.zero		-- 设置 width 和 height
